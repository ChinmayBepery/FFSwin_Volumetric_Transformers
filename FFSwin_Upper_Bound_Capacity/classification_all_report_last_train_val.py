...
font_prop = font_manager.FontProperties(
    family=FONT_FAMILY, size=BASE_FONT_SIZE, weight="bold"
)
# ===============================================
...
def run_master_thesis_evaluation():
    ...
    data_dir = r"G:\My Drive\OCT_Project_PhD_Implementation\BanglaOCT2025_Dataset\local_data_clean"
    model_path = r"G:\My Drive\OCT_Project_PhD_Implementation\FFSwin_Pseudo_3D_Model_main\models\classifier_best_100_used_here_as_final.pth"
    ...
    y_true, y_pred, y_probs, features_list=[],[],[],[]
    ...
    y_probs=np.array(y_probs); features_list=np.array(features_list)
    ...
    ax=sns.heatmap(cm,annot=True,fmt='d',cmap='Blues',cbar=True,annot_kws={"size":13,"weight":"bold"},
                   xticklabels=classes,yticklabels=classes)
    ax.set_xlabel("Predicted Label", fontproperties=font_prop, labelpad=10)
    ax.set_ylabel("True Label", fontproperties=font_prop, labelpad=10)
    ax.set_xticklabels(classes, rotation=0, fontproperties=font_prop)
    ax.set_yticklabels(classes, rotation=0, fontproperties=font_prop)
    ax.set_title(f"Confusion Matrix\nAccuracy = {acc*100:.2f}%", fontproperties=font_prop, pad=12)
    plt.tight_layout()
    ...
    for i,color in zip(range(n_classes),colors):
        ...
        plt.plot(fpr[i], tpr[i], color=color, lw=2, label=f'{classes[i]} (AUC={roc_auc[i]:.4f})')
    plt.plot([0,1],[0,1],'k--',lw=2)
    plt.xlim([0,1]); plt.ylim([0,1.05])
    plt.xlabel('False Positive Rate (1 - Specificity)', fontproperties=font_prop)
    plt.ylabel('True Positive Rate (Sensitivity)', fontproperties=font_prop)
    plt.title('Receiver Operating Characteristic (ROC) - Multi-Class', fontproperties=font_prop)
    ...
    plt.legend(handles, classes, title="Classes", prop=font_prop)
    plt.title("t-SNE Visualization of FFSwin Feature Space", fontproperties=font_prop)
    ...
    print("\n🎉 DONE! All figures generated and saved.")
