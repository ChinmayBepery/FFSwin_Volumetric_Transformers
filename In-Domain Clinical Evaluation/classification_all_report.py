...
# ==========================================
# 1. MODEL ARCHITECTURE (With Feature Extraction)
# ==========================================
# I modified the model slightly to return "Features" for t-SNE visualization
class Mlp(nn.Module):
...
class FFSwinClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x).flatten(1)  # (B, C) <- These are the Latent Features
...
# ==========================================
# 2. DATASET (Using Correct Folder Names)
# ==========================================
...
# ==========================================
# 3. HELPER: SPECIFICITY CALCULATION
# ==========================================
...
def run_master_thesis_evaluation():
    ...
    # 1. PATHS 
    data_dir = r"clean_data_train_valid_test\testdata"
    best_path = r"C:\Users\CSE-AI-Lab\Desktop\ChinmayResearch\PseudoFFSwin_Train_vali_test\checkpoints\classifier_best.pth"
    final_path = r"C:\Users\CSE-AI-Lab\Desktop\ChinmayResearch\PseudoFFSwin_Train_vali_test\checkpoints\classifier_best.pth"
    ...
    if len(ds) == 0:
        print("❌ Error: Dataset empty. Check folder names (DryAMD/WetAMD/NonAMD).")
        return
    dl = DataLoader(ds, batch_size=1, shuffle=False)
    
    # 3. LOAD MODEL
    # Set return_features=True for t-SNE
    model = FFSwinClassifier(num_classes=3, return_features=True).to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()
    
    # 4. RUN INFERENCE & COMPUTATIONAL STATS
    y_true = []
    y_pred = []
    y_probs = [] # For ROC
    features_list = [] # For t-SNE
    
    print(f"⏳ Processing {len(ds)} patients (Measuring Inference Time)...")
    ...
    avg_inference_time = (total_time / len(ds)) * 1000 # ms
    ...
    # ==========================================
    # REPORT 1: COMPUTATIONAL EFFICIENCY
    # ==========================================
    params = sum(p.numel() for p in model.parameters())
    file_size = os.path.getsize(model_path) / (1024*1024)
    
    print("\n" + "="*50)
    print("⚡ 1. COMPUTATIONAL EFFICIENCY REPORT")
    print("="*50)
    ...
    # ==========================================
    # REPORT 2: CLINICAL METRICS
    # ==========================================
    cm = confusion_matrix(y_true, y_pred)
    acc = accuracy_score(y_true, y_pred)
    specs = calculate_specificity(cm) # Class-wise
    
    print("\n" + "="*50)
    print("📊 2. CLINICAL METRICS REPORT")
    print("="*50)
    ...
    print(f"{'Class':<10} | {'Precision':<10} | {'Recall (Sens)':<12} | {'Specificity':<10}")
    print("-" * 50)
    ...
    print("✅ ROC Curves saved as 'roc_curves_thesis.png'")
    # plt.show() # Uncomment to show inline

    # ==========================================
    # REPORT 4: t-SNE FEATURE VISUALIZATION
    # ==========================================
    print("\nGenerating t-SNE Feature Map (This takes a moment)...")
    
    # We use the features extracted before the final classification layer
    # If dataset is huge, sample it. For ~857 pts, it's fine.
    ...
    plt.title("t-SNE Visualization of FFSwin Feature Space")
    plt.xlabel("t-SNE Dimension 1")
    plt.ylabel("t-SNE Dimension 2")
    plt.grid(True, alpha=0.3)
    plt.savefig('tsne_plot_thesis.png', dpi=300)
    print("✅ t-SNE Plot saved as 'tsne_plot_thesis.png'")
    print("\n🎉 DONE! All thesis artifacts generated.")
